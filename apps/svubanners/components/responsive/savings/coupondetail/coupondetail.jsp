...
<%
%><%@page session="false"
	import="com.svu.bannersites.cq5.components.savings.CouponSearchResults,com.adobe.granite.security.user.UserProperties,java.util.Calendar,java.util.Date,java.text.SimpleDateFormat,java.text.DateFormat"%>
			...
			<div>
				<div id="ecoupon-list" <c:if test="${couponSearchResults.grid }">class="grid"</c:if> >
					<c:forEach var="coupon" items="${couponSearchResults.coupons}" varStatus="status">
                         <c:set var="startOn" value="${coupon.startOn}"/>
...
<script>
 ...
 <c:forEach var="coupon" items="${couponSearchResults.coupons}" varStatus="status">
     ...
     <fmt:formatDate pattern="MM/dd/yy" value="${coupon.startOn}" var="start" />
	 <fmt:formatDate pattern="MM/dd/yy" value="${coupon.endOn}" var="end"  />

         var countDownTill = "${end}";
         $('#days').text(moment(countDownTill).diff(moment(), 'days') + " Days Left"); 
