...
<c:if test="${dailyDealsProduct.shouldRender }" >
  <c:set var="cleanProductName" value="${fn:replace(dailyDealsProduct.productname, ' ', '')}" />
  <c:set var="singleQuote" value="'" />
  <c:set var="doubleQuote" value='"' />
  <c:set var="cleanProductName" value="${fn:replace(cleanProductName, '&', '-')}" />
  <c:set var="cleanProductName" value="${fn:replace(cleanProductName, singleQuote, '')}" />
  <c:set var="cleanProductName" value="${fn:replace(cleanProductName, doubleQuote, '')}" />
  <c:set var="cleanProductName" value="${fn:replace(cleanProductName, ',', '_')}" />
    ...
    <div class="item-info">
      ...
      <div class="item-recipe">
        ...
        <c:forEach var="item2" items="${dailyDealsProduct.recipeid}" varStatus="row">
          ...
          <p><a href = "/content/svubanners/albertsons/en/recipes/view-recipe.${item2}.html" /> ${cmbIndex2[row.index]}</a> </p>
        </c:forEach>     
      </div>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
        $('#add-item-link-${cleanProductName}').click(function(evt){
            evt.preventDefault();
            $.ajax({
                  type: 'POST',
                  cache: false,
