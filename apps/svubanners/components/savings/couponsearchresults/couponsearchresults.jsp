...
<c:set var="params"
	value="?keyword=${couponSearchResults.keyword }&category=${couponSearchResults.category }&type=${couponSearchResults.type}&sortDirection=${couponSearchResults.sortDirection }&sortProperty=${couponSearchResults.sortProperty }" />
	...
	<div id="error-message-container" style="display: block;">
		<ul>
			<c:forEach var="message"
				items="${couponSearchResults.errorMessages }">
				<li>${message }</li>
			</c:forEach>
		</ul>
	</div>
...
<div class="sort">
	<div id="frm-coupon-search-options">
		<form method="get" action="${couponSearchResults.resultsPath}.html">
			<fieldset>
				<legend class="visually-hidden">Show Coupons</legend>
				<input type="hidden" name="keyword"
					value="${fn:escapeXml(couponSearchResults.keyword) }" />
				...
				<div class="filter-col">
					...
					<select aria-required="true" id="coupon-type" name="type">
						<option title="Save to account" value="LoyaltyOffer" <c:if test="${couponSearchResults.type=='LoyaltyOffer' }">selected="selected"</c:if>>Save
							to account</option>
						<option title="Print coupons" value="BarcodeSettlementOffer" <c:if test="${couponSearchResults.type=='BarcodeSettlementOffer' }">selected="selected"</c:if>>Print coupons</option>
					</select>
				</div>
				<div class="filter-col sort-col">
					<label for="sortProperty">sort coupons by:</label> <select
						id="sortProperty" name="sortProperty">
						<option value="">None</option>
						<option value="relevant"
							<c:if test="${couponSearchResults.sortProperty == 'relevant' }">selected=selected</c:if>>Relevance</option>
						<option value="title"
							<c:if test="${couponSearchResults.sortProperty == 'title' }">selected=selected</c:if>>Item</option>
						<option value="endOn"
							<c:if test="${couponSearchResults.sortProperty == 'endOn' }">selected=selected</c:if>>Expiration</option>
						<option value="value"
							<c:if test="${couponSearchResults.sortProperty == 'value' }">selected=selected</c:if>>Estimated
							Value</option>
					</select>
				</div>
				<input type="submit" id="searchCoupons" class="button"
					title="Search"
					onclick="//_gaq.push(['_trackEvent', '${currentPage.title }-keyword', 'couponsearchform', 'Search']);" value="" />
			</fieldset>
		</form>
		...
		<ul id="coupons-filter-results-info">
			...
			<c:choose>
				<c:when test="${couponSearchResults.pageMax == 6 }">
					<li>Show 6</li>
					<li><a
						href="${currentPage.path }.html${params }&displayCount=18">18</a></li>
					<li class="last"><a
						href="${currentPage.path }.html${params }&displayCount=48">48</a></li>
				</c:when>
				<c:when test="${couponSearchResults.pageMax == 18 }">
					<li>Show <a
						href="${currentPage.path }.html${params }&displayCount=6">6</a></li>
					<li>18</li>
					<li class="last"><a
						href="${currentPage.path }.html${params }&displayCount=48">48</a></li>
				</c:when>
				...
				<c:when test="${couponSearchResults.pageMax == 25 }">
					<li>Show <a
						href="${currentPage.path }.html${params }&displayCount=10">10</a></li>
					<li>25</li>
					<li class="last"><a
						href="${currentPage.path }.html${params }&displayCount=50">50</a></li>
				</c:when>
				<c:when test="${couponSearchResults.pageMax == 50 }">
					<li>Show <a
						href="${currentPage.path }.html${params }&displayCount=10">10</a></li>
					<li><a
						href="${currentPage.path }.html${params }&displayCount=25">25</a></li>
					<li class="last">50</li>
				</c:when>
				<c:otherwise>
					<li>Show 10</li>
	...
	<c:choose>
		...
		<c:otherwise>
			<div class="results-count">Results
				${couponSearchResults.fromIndex + 1 } to
				${couponSearchResults.toIndex } of ${couponSearchResults.totalCount }
			</div>
			<div>
							...
							<div class="coupon-options">
								...
								<p class="legalterms-container">
                                    <c:if test="${coupon.hasLegalTerms}">
                                        ...
                                        href="${currentPage.path}/coupon-detail.html?couponId=${coupon.couponId}">Details</a> 
                                    </c:if>
								</p>
									<form method="breadcrump.jsp" action="post">
				...
				<div class="pagination">
					<ul>
						<li class="previous"><c:if
								test="${couponSearchResults.pageNumber > 1}">
								<a
									href="${couponSearchResults.page.path}.html?displayCount=${couponSearchResults.pageMax}&currentPageNumber=${couponSearchResults.pageNumber - 1}&keyword=${couponSearchResults.keyword}&category=${couponSearchResults.category}&type=${couponSearchResults.type}&sortProperty=${couponSearchResults.sortProperty}">
							</c:if>Previous<c:if test="${couponSearchResults.pageNumber > 1}">
								</a>
							</c:if></li>
						<li class="pages">Page:
							<ol>
								<c:forEach var="pageIndex"
									items="${couponSearchResults.pagesToShow}">
									<c:if
										test="${((couponSearchResults.pageNumber + 3) < couponSearchResults.totalPages) && (pageIndex == couponSearchResults.totalPages) && (pageIndex != couponSearchResults.pageNumber)}">
										<li>...</li>
									</c:if>
									<c:choose>
									...
									<c:if test="${pageIndex != couponSearchResults.pageNumber}">
										<a
											href="${couponSearchResults.page.path}.html?displayCount=${couponSearchResults.pageMax}&currentPageNumber=${pageIndex}&keyword=${couponSearchResults.keyword}&category=${couponSearchResults.category}&type=${couponSearchResults.type}&sortProperty=${couponSearchResults.sortProperty}">
									</c:if>${pageIndex}<c:if
										test="${pageIndex != couponSearchResults.pageNumber}">
										</a>
									</c:if>
						</li>
						<c:if
							test="${((couponSearchResults.pageNumber - 3) > 1) && (pageIndex == 1)}">
							<li>...</li>
						</c:if>
						</c:forEach>
						</ol>
						...
						<li class="next"><c:if
								test="${couponSearchResults.pageNumber != couponSearchResults.totalPages}">
								<a
									href="${couponSearchResults.page.path}.html?displayCount=${couponSearchResults.pageMax}&currentPageNumber=${couponSearchResults.pageNumber + 1}&keyword=${couponSearchResults.keyword}&category=${couponSearchResults.category}&type=${couponSearchResults.type}&sortProperty=${couponSearchResults.sortProperty}">
							</c:if>Next<c:if
								test="${couponSearchResults.pageNumber != couponSearchResults.totalPages}">
